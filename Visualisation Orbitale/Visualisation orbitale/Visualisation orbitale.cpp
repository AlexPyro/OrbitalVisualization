...
GLfloat cameraPosZ = 20.0f;  // Augmenter le recul initial
...
const char* input_file = "smooth1pos.txt";
const char* input_file2 = "smoothrefpos.txt";
...
std::vector<Point> readEllipsePoints(const char* filename)
{
    ...
    while (inputFile >> a >> b >> d >> e >> x >> y >> z >> f >> g >> h >> i >> j >> k >> l >> m >> n >> o >> p >> q >> r)
    {
        points.push_back({ x/1000, y/1000, z/1000 });
    }
...
void displaySphereEllipse()
{
    ...
    //glutWireSphere(6.371, 360, 200);
    glutSolidSphere(6.371, 50, 50);
    glPopMatrix();
    ...
    glBegin(GL_LINE_STRIP); // Relie les points entre eux par des lignes
    for (const auto& point : readEllipsePoints(input_file))
    {
    ...
    for (const auto& point : readEllipsePoints(input_file2))
    {
        glVertex3f(GLfloat(point.x), GLfloat(point.y), GLfloat(point.z));
    }
...
int main(int argc, char** argv) 
{
    ...
    //generateEllipsePoints("ellipse_coordinates.txt", 5.0, 3.0, 3.0, 100);
    //generateEllipsePoints("ellipse_coordinates2.txt", 6.0, 4.0, 10.0, 100);
